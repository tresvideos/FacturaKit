...
const Input = ({ className = "", ...props }: any) => (
  <input className={`w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 shadow-sm focus:border-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-900/10 ${className}`} {...props} />
);
const Select = ({ className = "", children, ...props }: any) => (
  <select className={`w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm focus:border-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-900/10 ${className}`} {...props}>{children}</select>
);
const Textarea = ({ className = "", ...props }: any) => (
  <textarea className={`w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 shadow-sm focus:border-slate-900 focus:outline-none focus:ring-2 focus:ring-slate-900/10 ${className}`} {...props} />
);
...
const Modal = ({ open, onClose, title, children, wide = false }: any) => !open ? null : (
  <div className="fixed inset-0 z-50 flex items-center justify-center">
    ...
    <div className={`relative z-10 maxh-[90vh] overflow-auto rounded-2xl bg-white shadow-2xl ${wide?"w-[1080px]":"w-[720px]"}`}>
...
function useAuth(){
  ...
  const signup=(email:string,password:string)=>{const s=getStore(); s.users=s.users||{}; if(s.users[email]) throw new Error("Este correo ya está registrado."); s.users[email]={email,password,invoices:[],plan:{name:"free",remaining:3,max:3}}; s.currentUser={email}; setStore(s); setUser({email})}
  const login=(email:string,password:string)=>{const s=getStore(); if(!s.users||!s.users[email]) throw new Error("Usuario no encontrado."); if(s.users[email].password!==password) throw new Error("Contraseña incorrecta."); s.currentUser={email}; setStore(s); setUser({email})}
  ...
  const setUserData=(updater:any)=>{const s=getStore(); if(!user) return; const d=s.users?.[user.email]; s.users[user.email]=typeof updater==="function"?updater(d):updater; setStore(s)}
...
function Header({ onGoto, user, onOpenContact }: any){
  return (<header className="sticky top-0 z-40 w-full bg-white/80 backdrop-blur border-b border-slate-200">
    <Container><div className="flex h-16 items-center justify-between">
      <div className="flex items-center gap-3"><div className="h-9 w-9 rounded-xl bg-slate-900 text-white grid place-items-center font-bold">F</div><span className="hidden text-sm font-semibold text-slate-900 sm:block">Facturas V7</span></div>
...
function Home({ onGoto }: any){
  return (<section className="bg-gradient-to-b from-slate-50 to-white">
    <Container className="py-16 sm:py-24">
      <div className="grid items-center gap-10 lg:grid-cols-2">
        <div>
          <Badge>Nuevo flujo</Badge>
          <h1 className="mt-4 text-4xl font-extrabold tracking-tight text-slate-900 sm:text-5xl">Elige tu plantilla. Diseña. Descarga.</h1>
          <p className="mt-4 text-lg text-slate-600">Previews reales, editor completo y <span className="font-semibold">3 facturas gratis</span>.</p>
          <div className="mt-6 flex flex-wrap gap-3">
            <Button onClick={()=>onGoto("templates")} variant="success">Empezar</Button>
            <Button variant="secondary" onClick={()=>onGoto("templates")}>Ver plantillas</Button>
          </div>
        </div>
        <div><TemplatesStrip/></div>
      </div>
    </Container>
  </section>)
}
const TemplatesStrip=()=> (<div className="grid grid-cols-5 gap-2">{TEMPLATES.map(t=>(<div key={t.id} className={`h-20 rounded-xl ${t.style} text-white grid place-items-center text-xs font-semibold`}>{t.name}</div>))}</div>)
  ...
  return (<Container className="py-12">
    ...
    <div className="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-5">
      {TEMPLATES.map(t=>(<Card key={t.id}><CardHeader title={t.name} subtitle={t.vibe} right={<Badge>Demo</Badge>}/><CardBody>
        <div className={`mb-4 h-32 w-full rounded-xl ${t.style} grid place-items-center text-white font-semibold`}>{t.name}</div>
        <div className="flex items-center justify-between gap-2"><Button variant="secondary" onClick={()=>onPreviewTemplate(t)}>Ver ejemplo</Button><Button onClick={()=>onPick(t)}>Usar</Button></div>
      </CardBody></Card>))}
    </div>
  </Container>)
}
...
function Builder({ template, initial, onBack, onProceed, auth }: any){
  ...
  return (<Container className="py-8">
    <div className="mb-6 flex items-center gap-3"><Button variant="secondary" onClick={onBack}>Volver</Button><Badge>Plantilla: {template?.name}</Badge></div>
    <div className="grid gap-6 lg:grid-cols-2">
      <Card><CardHeader title="Datos de la factura" subtitle="Completa todos los campos necesarios"/><CardBody className="space-y-5">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div><label className="text-xs font-medium text-slate-600">Color de acento</label><div className="flex flex-wrap gap-2">{(template?.colors||[]).map((c:string)=>(<button key={c} className={`h-8 w-8 rounded-full ring-2 ${inv.color===c?"ring-slate-900":"ring-transparent"}`} style={{background:c}} onClick={()=>change({color:c})} type="button" />))}</div></div>
          <div/>
        </div>
        <div className="grid grid-cols-2 gap-3">
      ...
      <div className="space-y-3">
        <Card><CardHeader title="Previsualización" subtitle="Actualiza en tiempo real"/><CardBody><InvoiceDoc invoice={inv} accentColor={inv.color}/></CardBody></Card>
...
function InvoiceDoc({invoice,accentColor}: any){
  const items=invoice.items||[]; const totals=calcTotals(items,invoice.discount,invoice.taxRate); const accent={color:accentColor||"#0f172a"}
  return (<div className="rounded-2xl border border-slate-200 p-6">
    <div className="flex items-start justify-between gap-6">
      <div><div className="inline-block rounded-full px-2 py-0.5 text-xs text-white" style={{background:accent.color}}>Factura</div>
    ...
    <div className="mt-6 overflow-x-auto">
      <table className="min-w-full text-sm"><thead><tr className="text-slate-600"><th className="w-2/3">Descripción</th><th>Cant.</th><th>Precio</th><th>Importe</th></tr></thead>
      <tbody>{items.map((it:any)=>(<tr key={it.id}><td>{it.description}</td><td>{it.qty}</td><td>{currency(it.price)}</td><td>{currency(Number(it.qty)*Number(it.price))}</td></tr>))}</tbody></table>
    </div>
    ...
    {(invoice.notes||invoice.terms) && (<div className="mt-6 grid gap-4 text-xs md:grid-cols-2">
...
function Dashboard({ auth, onOpenContact, onGotoBuilder, onRename }: any){
  ...
  return (<Container className="py-10">
    ...
    <Card>
      ...
      {tab==="invoices" && (<CardBody>{invoices.length===0?(<EmptyInvoices onCreate={onGotoBuilder}/>):(<div className="grid gap-4">
        {invoices.map((inv:any)=>(<div key={inv.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 p-4">
...
function HelpPlans({ auth }: any){
  ...
  return (<CardBody className="space-y-6">
    ...
    <div><h4 className="text-lg font-semibold text-slate-900 mb-3">Tu suscripción</h4>
...
function InvoicePreviewModal({ open, onClose, invoice }: any){
  ...
  const html=()=>`<!doctype html><html><head><meta charset='utf-8'><title>Factura ${invoice?.number}</title><style>${PRINT_STYLES}</style></head><body>${areaRef.current?.innerHTML||""}</body></html>`
  ...
  return (<Modal open={open} onClose={onClose} title={`Factura #${invoice.number}`} wide>
    ...
    <div ref={areaRef}><InvoiceDoc invoice={invoice} accentColor={invoice.color}/></div>
  </Modal>)
}
...
export default function App(){
  const auth=useAuth(); const [route,setRoute]=useState("home"); const [contactOpen,setContactOpen]=useState(false)
  const [templateModal,setTemplateModal]=useState<any>(null); const [pickedTemplate,setPickedTemplate]=useState<any>(null); const [builderInvoice,setBuilderInvoice]=useState<any>(null); const [authGateOpen,setAuthGateOpen]=useState(false); const [successModal,setSuccessModal]=useState<any>(null)
  ...
  return (<div className="min-h-screen bg-white text-slate-900">
    ...
    <Footer/>
    <Modal open={Boolean(templateModal)} onClose={()=>setTemplateModal(null)} title={`Ejemplo · ${templateModal?.name}`} wide>{templateModal && (<div className="space-y-4"><InvoiceDoc invoice={{...DEFAULT_SAMPLE, templateId: templateModal.id, color: templateModal.colors[0]}} accentColor={templateModal.colors[0]}/><div className="flex justify-end"><Button onClick={()=>{setPickedTemplate(templateModal); setTemplateModal(null); setRoute("builder")}}>Usar esta plantilla</Button></div></div>)}</Modal>
    ...
    <Modal open={Boolean(successModal) && route!=="dashboard"} onClose={()=>setSuccessModal(null)} title="¡Felicidades!" wide>
      ...
      <div className="my-4">{successModal && <InvoiceDoc invoice={successModal} accentColor={successModal.color}/>}</div>
...
function Footer(){ return (<footer className="border-t border-slate-200 py-10"><Container><div className="flex flex-col items-center justify-between gap-4 sm:flex-row"><p className="text-sm text-slate-600">© {new Date().getFullYear()} Facturas V7. Todos los derechos reservados.</p><div className="flex items-center gap-4 text-sm"><a className="text-slate-600 hover:text-slate-900" href="#">Términos</a><a className="text-slate-600 hover:text-slate-900" href="#">Privacidad</a></div></div></Container></footer>) }
